...
const MODES_PAIEMENT = ['virement', 'cheque', 'especes', 'carte', 'prelevement', 'autre'];
...
export default function MicroFactureForm() {
  const { id: clientId } = useParams();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const devisSourceId = searchParams.get('from_devis');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [cRes, pRes, numRes] = await Promise.all([
        ...
        api.get(`/micro-factures/next-numero/${mcRes.data.id}`),
      ]);
  ...
  const save = async (sendAfter = false) => {
    ...
    try {
      ...
      const res = await api.post('/micro-factures', payload);
      const factureId = res.data.id;

      if (sendAfter) {
        try {
          await api.post(`/micro-factures/${factureId}/envoyer`);
        } catch (e) {
          alert('Facture créée mais erreur d\'envoi : ' + (e.response?.data?.error || e.message));
        }
      }

      navigate(`/clients/${clientId}/micro/factures/${factureId}`);
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 24px' }}>
      ...
      <h1 style={{ margin: '0 0 8px', fontSize: 22, fontWeight: 700 }}>Nouvelle facture</h1>
      ...
      {step === 3 && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div>
              <label style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Numéro de facture</label>
