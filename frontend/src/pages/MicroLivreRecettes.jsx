...
export default function MicroLivreRecettes() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (mcRes.data) {
        const res = await api.get(`/micro-factures/livre-recettes/${mcRes.data.id}?annee=${annee}`);
        setData(res.data);
      }
    } catch (e) {
      console.error(e);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `Livre_Recettes_${microClient?.nom_commercial || clientId}_${annee}.csv`;
    a.click();
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '28px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#6b7280', fontSize: 13 }}>
            {microClient?.nom_commercial || '—'} · Exercice {annee}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={exportCSV} disabled={!data.rows.length}
            style={{ padding: '8px 16px', background: '#fff', color: '#0F1F4B', border: '1px solid #d1d5db', borderRadius: 6, cursor: data.rows.length ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 600, opacity: data.rows.length ? 1 : 0.5 }}>
            ⬇ CSV
          </button>
          <button
            onClick={() => window.open(`/api/micro-factures/livre-recettes/${microClient.id}/pdf?annee=${annee}&token=${token}`, '_blank')}
            disabled={!data.rows.length || !microClient}
            style={{ padding: '8px 16px', background: '#0F1F4B', color: '#fff', border: 'none', borderRadius: 6, cursor: data.rows.length ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 600, opacity: data.rows.length ? 1 : 0.5 }}>
            ⬇ PDF
          </button>
        </div>
      </div>

      {/* Résumé trimestriel — base déclaration URSSAF */}
      ...
      {/* Total annuel */}
      <div style={{ background: '#0F1F4B', borderRadius: 10, padding: '16px 22px', marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
        {data.rows.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center', color: '#9ca3af' }}>
            ...
            <div style={{ fontSize: 13 }}>Les entrées sont créées automatiquement lors de l'enregistrement de paiements.</div>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#0F1F4B' }}>
                {['#', 'Date', 'N° Facture', 'Client', 'Nature de la prestation', 'Montant encaissé', 'Mode'].map(h => (
                  <th key={h} style={{
                    padding: '10px 14px', textAlign: h === 'Montant encaissé' ? 'right' : 'left',
            ...
            <tbody>
              {data.rows.map((r, i) => (
                <tr key={r.id} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 ? '#fafafa' : '#fff' }}>
                  ...
                  <td style={{ padding: '10px 14px', whiteSpace: 'nowrap' }}>{fmtDate(r.date_encaissement)}</td>
            ...
            <tfoot>
              <tr style={{ background: '#f9fafb', borderTop: '2px solid #e5e7eb' }}>
                ...
                <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: 700, fontSize: 15, color: '#0F1F4B' }}>
                  {fmtEur(data.total)}
                </td>
                <td />
              </tr>
      ...
      <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 16, textAlign: 'center' }}>
        Livre des recettes généré automatiquement · Données conformes aux obligations BOFiP pour le régime micro-entrepreneur · Aucune suppression possible
      </p>
    </div>
  );
}
